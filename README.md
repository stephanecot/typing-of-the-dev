...
- `ÉCHAP` relâche la cible (sa saisie est remise à zéro). `F2` coupe le son.
...
| | STAGIAIRE | DEV CONFIRMÉ | SENIOR 10X |
|---|---|---|---|
| Vitesse | ×0.62 | ×1.0 | ×1.35 |
| Incidents tolérés | 4 | 3 | 2 |
| Multiplicateur score | ×1 | ×1.5 | ×2 |
| Commandes du boss | 2 | 3 | 4 |

## Base de données & exports

SQLite dans `db/typing-of-the-dev.sqlite` (créée au premier lancement).
Chaque partie enregistre : pseudo, email *(optionnel, avec consentement RGPD)*,
difficulté, score, sprints, WPM, précision, combo max, durée, kills par type de
monstre et mots ratés.
